...
import { getUserBookmarks, createUserBookmark, deleteUserBookmark } from '@/lib/bookmark-utils'
...
const handler = createMcpHandler((server) => {
  ...
  server.tool(
    "create-bookmark",
    "create a new bookmark for an authenticated user",
    {
        url: z.string().describe("The URL of the bookmark to create"),
        title: z.string().describe("The title of the bookmark"),
        notes: z.string().optional().describe("Additional notes for the bookmark"),
    },
    async (args, { authInfo }) => {
        try{
            const userId = authInfo!.extra!.userId! as string;
  ...
  server.tool(
    "delete-bookmark",
    "delete a bookmark for an authenticated user",
    {
    ...
    async (args, { authInfo }) => {
        try{
            const userId = authInfo!.extra!.userId! as string;
            const success = await deleteUserBookmark(userId, args.bookmarkId);
            ...
            if (success) {
                return {
                    content: [
                        {
                            type: "text",
                            text: `Successfully deleted bookmark with ID: ${args.bookmarkId}`,
                        },
                    ],
                };
            } else {
                return {
                    content: [
                        {
                            type: "text",
                            text: `Failed to delete bookmark with ID: ${args.bookmarkId}`,
                        },
                    ],
                };
            }
